    ...
    if not run or (run == "n"):
        ...
        with open(wordlist, "r") as wordlist_file:
            for line in wordlist_file:
                word = line.strip()
                test_url = "https://" + word + "." + target_url
                response = request(test_url)
                if response:
                    print("\033[0;32;40m[+] Discovered subdomain --> " + test_url)
            ...
            for new_directory in discovered_directories:
                ...
                with open(wordlist, "r") as new_directory_file:
                    for l in new_directory_file:
                        ...
                        if response:
                            print("\033[0;32;40m[+] Discovered a new URL --> " + new_test_directory)
...
def file_discovery(target_url, wordlist, filelist):
    ...
    if not run or (run == "n"):
        ...
        extension = input("--> ")
        if not filelist:
            print("\033[0;33;40m[+]No File List specified, using the same wordlist: " + wordlist + " [+]")
            wlist = wordlist
        else:
            ...
            with open(wlist, "r") as filename_file:
                for line in filename_file:
                    filename = line.strip()
                    test_file = target_url + "/" + filename + extension 
                    file_reach = requests.get(test_file)
                    if file_reach:
                        print("\033[0;32;40m[+] Discovered File--> " + test_file)
